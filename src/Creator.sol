...
contract Creator is Ownable, ReentrancyGuard, Pausable {
    ...
    function acceptAllDonations() external onlyOwner nonReentrant whenNotPaused {
        uint96 totalTransfer = 0;
        uint96 totalFee = 0;
        ...
        for (uint256 i = 0; i < donations.length; i++) {
            ...
            if (!donation.isAccepted && !donation.isBurned) {
                ...
                uint96 fee = feePerDonation;
                uint96 donationAmount = donation.amount;
                uint96 creatorAmount = donationAmount - fee;

                totalTransfer += creatorAmount;
                totalFee += fee;

                emit DonationAccepted(i);
            }
        }
        ...
        if (totalTransfer > 0) {
            (bool success,) = owner().call{value: totalTransfer}("");
            if (!success) revert("Transfer failed");
        }

        totalPendingAmount -= (totalTransfer + totalFee);
    }
